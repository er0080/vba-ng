...
| VBA0002 | error; warning for a `.frm` | Valid VBA that this version of the compiler does not carry: a `Declare` whose calling convention, parameter, or return type needs its own marshaling, a procedure `AddressOf` cannot hand over as a callback, an `As Any` parameter with no fixed-size argument, `Me` in a standard module, an unimplemented intrinsic, or a `MidB` assignment. A `.frm` is the one warning: the project still builds and the form's code compiles as a class whose controls are Nothing. | `Declare PtrSafe Sub F Lib "k" (ParamArray a() As Variant)`; any `.frm` for the warning |
...
| VBA0023 | error | Can't find project or library: a manifest reference whose guid is registered nowhere and whose name is not one of the libraries vba-ng knows (Excel, Office, Scripting, stdole, VBIDE, MSForms). | `{ "name": "Nope", "guid": "{00000000-0000-0000-0000-000000000001}", "version": "1.0" }` in `vbang.json` |
