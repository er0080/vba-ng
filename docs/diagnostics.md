...
| VBA0003 | error | Internal compiler error: the generated C# failed to compile. The message is `Internal compiler error <Roslyn id>: <Roslyn message>`, at the mapped VBA location. Always a compiler bug; report it with the module that caused it. | none: only a compiler bug raises it, so no test can |
