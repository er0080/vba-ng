...
Commit the sources and `vbang.json`. `out/` is derived: `vbang init` writes a `.gitignore`
containing `out/` for you. `vbang import` does not, so add the ignore yourself after an import.
...
referenced library that has a GUID, and a `documents` map of each document module and its kind. It
